            ...
            <>
              ...
              <div className="absolute left-1/2 -translate-x-1/2 top-full mt-1 bg-white border border-gray-200 rounded-xl shadow-lg z-20 w-44 max-h-60 overflow-y-auto">
                {dropdownMonths.map(({ year, month, label, weeks }) => (
                  <div key={`${year}-${month}`}>
                    <div className="px-3 py-2 text-xs font-semibold text-gray-400 bg-gray-50 sticky top-0">
                      {label}
                    </div>
                    {weeks.map(({ label: wLabel, firstDay }) => {
                      ...
                      return (
                        <button
                          key={wLabel}
                          onClick={() => { onDateSelect(firstDay); setDropdownOpen(false) }}
        ...
        {days.map((day, i) => {
          ...
          return (
            ...
            >
              ...
              <span className={`text-xs font-medium min-h-4
